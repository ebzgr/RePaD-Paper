...
def check_probability_distribution(N, expname, periodlist, true_partition=False):
    ...
    for k in np.arange(N):
        print(k)
        if(true_partition):
            parts = pd.read_pickle("data/{}/partition_{}.pickle".format(expname,k))
        for ttype_ext in [3,15]:
            for m_trans in [False,True]:
                for periods in periodlist:
                    train_df = pd.read_pickle('data/{}/train_df_{}_{}_{}_{}.pickle'.format(expname,k,ttype_ext,m_trans*1,periods))
                    valid_df = pd.read_pickle('data/{}/valid_df_{}_{}_{}_{}.pickle'.format(expname,k,ttype_ext,m_trans*1,periods))
...
def simulation_analysis(n, dim_q, periodlist, expname):
...
def single_run(data, val_data, k, ttype_ext, m_trans, periods, dim_pi, lamb, expname):
    ...
    parts.to_pickle("data/{}/parts_{}_{}_{}_{}_{}.pickle".format(expname, k,ttype_ext,m_trans*1,periods,lamb))
    report.to_pickle("data/{}/report_{}_{}_{}_{}_{}.pickle".format(expname, k,ttype_ext,m_trans*1,periods,lamb))
...
def simulation(seed):
    ...
    for k in np.arange(start,n,4):
        ...
        partitions.to_pickle("data/{}/partition_{}.pickle".format(expname,k))
        partitions['f_dc'] = 0
        ...
        for ttype_ext in [int(dim_pi/5),dim_pi]:
            ...
            for m_trans in [False,True]:
                ...
                for periods in [400,100]:
                    ...
                    train_df.to_pickle('data/{}/train_df_{}_{}_{}_{}.pickle'.format(expname,k,ttype_ext,m_trans*1,periods))

                    valid_df = data_gen.generate(buses=buses, periods=periods)
                    val_data = utl.get_partitioning_variables(valid_df)
                    valid_df.to_pickle('data/{}/valid_df_{}_{}_{}_{}.pickle'.format(expname,k,ttype_ext,m_trans*1,periods))
    
                    for lamb in [0,0.2,0.5,1,2,5,100]:
                        single_run(data, val_data, k, ttype_ext, m_trans, periods, dim_pi, lamb, expname)
